...
import { BookOpen, ArrowLeft, CheckCircle2, XCircle, Play, Timer, Plus } from 'lucide-react';
  ...
  const startQuiz = async () => {
    try {
      ...
      if (allQuestions.length === 0) {
        setSuccessMessage("দুঃখিত, কোনো গাণিতিক অঙ্ক পাওয়া যায়নি।");
        return;
      }

      // Shuffling and Filtering Logic
      const storageKey = `seen_questions_word_problem`;
  ...
  if (state === 'setup') {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        >
          <div className="bg-purple-50 w-16 h-16 md:w-20 md:h-20 rounded-3xl flex items-center justify-center mb-6 md:mb-8">
            <BookOpen className="h-8 w-8 md:h-10 md:w-10 text-purple-500" />
          </div>
          <h1 className="text-2xl md:text-3xl font-display font-bold mb-6 md:mb-8">গাণিতিক অঙ্ক কুইজ সেটআপ</h1>
  ...
  if (state === 'playing') {
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        >
          <h2 className="text-2xl md:text-3xl font-display font-medium mb-12 leading-relaxed text-center">
            {question.question}
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6">
            {question.options.map((option, idx) => (
              <button
                ...
                className={`p-4 md:p-6 rounded-2xl border-2 text-xl md:text-2xl font-bold transition-all ${
